...
if {[file exists CIC.vo]} {
	set hdl_ext "v"
} else {
	puts "Warning: Could not find CIC.$ipfs_ext!"
}
...
# test bench settings
set_global_assignment -name EDA_DESIGN_INSTANCE_NAME CIC_inst -section_id tb
set_global_assignment -name EDA_TEST_BENCH_MODULE_NAME work.CIC_tb -section_id tb

# IPFS file
set_global_assignment -name EDA_IPFS_FILE "CIC.${ipfs_ext}" -section_id eda_simulation -library work

foreach i $input_files {
  set_global_assignment -name EDA_TEST_BENCH_FILE "$i" -section_id tb -library work
}
set_global_assignment -name EDA_TEST_BENCH_FILE "CIC_tb.${hdl_ext}" -section_id tb -library work
